...
RSpec.describe Api::V1::IdentifierTypesController, type: :controller do
  ...
  describe "#create" do
    ...
    it "creates a new identifier type" do
      post :create, name: 'myappdb_user_id'
    ...
    it 'is idempotent for existing identifier types' do
      ...
      FactoryGirl.create :identifier_type, owner_app: another_app, name: 'myappdb_user_id'

      expect { post :create, name: 'myappdb_user_id' }.not_to change { IdentifierType.count }
      expect(response).to have_http_status(:no_content)
      expect(IdentifierType.find_by(name: 'myappdb_user_id', owner_app: another_app)).to be_truthy
    end

    it 'returns errors when invalid' do
      post :create, name: 'myappdbUserId'
